...
- [x] **Kompilator Partytur — Fundamenty:** Kanoniczny schemat domeny (`Composer.mbid`, `Piece.mbid_work`, `ScoreEdition`, `Movement`, `Translation`, `Recording`, `Annotation`, `ProgramNote`, `ProvenanceRecord`), wielopoziomowy wrapper Claude z adaptacyjnym myśleniem + śledzeniem kosztów + buforowaniem promptów oraz buforowane w Redis klienty zewnętrzne (MusicBrainz, Wikidata, Spotify, YouTube).
- [ ] **Kompilator Partytur — Pipeline przetwarzania:** Łańcuch Celery ekstrahujący metadane PDF, rozwiązujący kompozytorów/utwory względem MusicBrainz, generujący noty programowe + IPA + przekłady śpiewalne i prezentujący ekran przeglądu dla dyrygenta.
- [ ] **Kompilator Partytur — Montaż koncertu:** Przepływ WeasyPrint + pypdf produkujący dopracowany skoroszyt koncertowy (okładka, spis treści, materiały wstępne per utwór, oryginalne partytury) na żądanie.
- [ ] **Kompilator Partytur — Edytor adnotacji:** Nakładka PDF.js + Konva do oznaczania w przeglądarce (podświetlenie, komentarz, odręczne rysowanie, zmiana kolejności stron) z wersjonowaną, warstwową persystencją i spłaszczaniem przy eksporcie.
