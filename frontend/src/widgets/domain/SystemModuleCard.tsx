...
export function SystemModuleCard({
  ...
  return (
    <GlassCard
      ...
      className="relative flex h-full min-h-[160px] w-full flex-col justify-between overflow-hidden p-6 transition-all duration-1000 ease-[0.16,1,0.3,1] focus-visible:ring-1 focus-visible:ring-ethereal-gold/50"
    >
      ...
      >
        <Heading
          as="span"
          weight="light"
          className="text-[8rem] leading-none tracking-tighter md:text-[10rem]"
        >
          {romanNumeral}
        </Heading>
      </div>
      ...
      <header className="relative z-10 flex flex-col gap-4">
        ...
        {features.length > 0 ? (
            ...
            {features.map((feature, idx) => (
              <li key={idx} className="flex items-center gap-2">
                ...
                <Eyebrow
                  as="span"
                  color="graphite"
                  className="opacity-50 transition-opacity duration-500 group-hover:opacity-80"
                >
                  {feature}
                </Eyebrow>
              </li>
          ...
          <Eyebrow
            as="p"
            color="graphite"
            className="max-w-[80%] opacity-60 leading-relaxed"
          >
            {description}
          </Eyebrow>
        )}
