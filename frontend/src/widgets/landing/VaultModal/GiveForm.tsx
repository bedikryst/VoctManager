 ...
 * 20/50/100, or custom amount), email + consent capture, then POSTs to the public
 * Django endpoint that returns a gateway redirect URL. Exposes a `preselect(amount)`
 * imperative API via VaultContext so opening links can pre-populate the form.
 * @architecture Enterprise SaaS 2026
 * @module widgets/landing/VaultModal/GiveForm
 */

import { useCallback, useEffect, useMemo, useRef, useState } from "react";

import { initiateDonation, DonationInitiateError } from "@/features/landing/api/donations";
import {
  CURRENCY_SUFFIX,
  GIVE_DEFAULT_TIER,
  GIVE_MAX,
  ...
  GIVE_TIERS,
  type GiveCurrency,
} from "@/features/landing/constants/giveTiers";
...
export function GiveForm(): React.JSX.Element {
  ...
  return (
    <form className="give-form" onSubmit={onSubmit} noValidate aria-busy={loading}>
      ...
      {/* setCurrency is part of the form contract; PLN/EUR toggle UI is currently
          commented out in the source HTML so we don't render the switcher, but the
          handler stays available for when it's re-enabled. */}
