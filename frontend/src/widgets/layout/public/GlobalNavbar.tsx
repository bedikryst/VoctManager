...
export default function GlobalNavbar({
  ...
  const logoVariants: Variants = {
    bare: {
      ...
      filter: "blur(4px)",
      pointerEvents: "none",
      transition: { duration: 0.6, ease: butteryEase },
    },
    top: {
      opacity: 1,
      y: 0,
  ...
  const donateVariants: Variants = {
    bare: {
      ...
      filter: "blur(4px)",
      pointerEvents: "none",
      transition: { duration: 0.8, ease: butteryEase },
    },
    top: {
      opacity: 1,
      x: 0,
    ...
    >
      ...
      >
        ...
        <motion.div className="flex-1 flex justify-start items-center">
          ...
          >
            <span
              className={`h-px ease-out transition-all duration-[0.6s] ${isBare && isDarkBg ? "bg-white" : "bg-stone-900"} ${isBare ? "w-9 group-hover:w-6" : isPill ? "w-7 group-hover:w-4" : "w-9 group-hover:w-6"}`}
            />
            <span
              className={`h-px ease-out transition-all duration-[0.6s] ${isBare && isDarkBg ? "bg-white" : "bg-stone-900"} ${isBare ? "w-7 group-hover:w-8" : isPill ? "w-5 group-hover:w-6" : "w-7 group-hover:w-8"}`}
            />
          </button>
        </motion.div>
        ...
        >
          ...
          >
            <span>V</span>
            <span
              className={`overflow-hidden flex items-center whitespace-nowrap transition-all duration-1000 ${windowData.isMobile && isPill ? "max-w-0 opacity-0" : "max-w-[100px] opacity-100"}`}
            >
              oct
            </span>
            <span>E</span>
        ...
        <motion.div className="flex-1 flex justify-end items-center">
          ...
          <Link
            to="/panel"
            className="group active:scale-90 transition-colors duration-500 flex items-center"
            aria-label="Access Client Panel"
          >
            <svg
          ...
          >
            <Link
              to="/wesprzyj"
              className="group active:scale-95 transition-transform flex w-full"
            >
              <div
                className={`relative flex items-center justify-center w-full border h-9 md:h-10 rounded-lg md:rounded-xl overflow-hidden transition-all duration-[1.2s] ${isPill ? "bg-stone-900 text-stone-100 border-transparent hover:bg-amber-700 hover:shadow-lg hover:-translate-y-0.5" : "bg-transparent border-stone-900/30 text-stone-900 hover:bg-stone-900 hover:text-stone-100 hover:border-transparent"}`}
              >
                {!windowData.isMobile && (
