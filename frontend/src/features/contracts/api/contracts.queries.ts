...
export const useContractLedgers = () => {
    const projectsQuery = useQuery({ queryKey: CONTRACT_QUERY_KEYS.projects, queryFn: ContractsService.getProjects });
    const castQuery = useQuery({ queryKey: CONTRACT_QUERY_KEYS.participations, queryFn: ContractsService.getParticipations });
    const crewQuery = useQuery({ queryKey: CONTRACT_QUERY_KEYS.crew, queryFn: ContractsService.getCrewAssignments });

    return {
        projects: projectsQuery.data || [],
        participations: castQuery.data || [],
        crewAssignments: crewQuery.data || [],
        isLoading: projectsQuery.isLoading || castQuery.isLoading || crewQuery.isLoading,
    ...
    >({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ 
                queryKey: type === 'CAST' ? CONTRACT_QUERY_KEYS.participations : CONTRACT_QUERY_KEYS.crew 
            });
        },
    });
};
...
export const useBulkUpdateFee = () => {
    ...
    return useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: CONTRACT_QUERY_KEYS.participations });
        },
    });
};