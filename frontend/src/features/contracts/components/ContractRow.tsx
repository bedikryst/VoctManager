...
export function ContractRow({ record, type, onDownload }: ContractRowProps): React.JSX.Element {
    ...
    const personName = type === 'CAST' ? (record as EnrichedParticipation).artist_name : (record as EnrichedCrewAssignment).collaborator_name;
    const roleDisplay = type === 'CAST' ? (record as EnrichedParticipation).artist_voice_type_display : (record as EnrichedCrewAssignment).collaborator_specialty_display;
    ...
    const handleSaveFee = async (): Promise<void> => {
        ...
        try {
            ...
            toast.error(`Failed to save remuneration for ${personName || 'contractor'}.`);
        }
    };
