...
export const useMicroCasting = (projectId: string) => {
  ...
  const { data: voiceLines = [] } = useQuery<VoiceLineOption[]>({
    ...
    queryFn: async () =>
      (await api.get<VoiceLineOption[]>("/api/options/voice-lines/")).data,
    staleTime: Infinity,
  });
  ...
  const { data: program = [] } = useQuery<ProgramItem[]>({
    ...
    queryFn: async () => {
      const res = await api.get<ProgramItem[]>(
  ...
  const handleDragEnd = async (event: DragEndEvent): Promise<void> => {
    ...
    const existingCasting = localCastings.find(
      (c) => String(c.participation) === partId,
    );
    ...
    if (targetVoiceLineId === "UNASSIGNED") {
      newCastings = newCastings.filter(
        (c) => String(c.participation) !== partId,
      );
    } else {
      ...
      if (existingCasting) {
        newCastings = newCastings.map((c) =>
          String(c.participation) === partId
            ? { ...c, voice_line: targetVoiceLine }
            : c,
        );
      } else {
        newCastings.push({
    ...
    try {
      if (targetVoiceLineId === "UNASSIGNED") {
        if (existingCasting?.id)
          await api.delete(`/api/piece-castings/${existingCasting.id}/`);
      } else {
        const targetVoiceLine = targetVoiceLineId as PieceCasting["voice_line"];
        if (
          existingCasting?.id &&
          ...
          await api.patch(`/api/piece-castings/${existingCasting.id}/`, {
            voice_line: targetVoiceLine,
          });
        } else {
      ...
      toast.error("Błąd synchronizacji", {
        description: "Nie udało się zaktualizować obsady.",
      });
    }
  };
