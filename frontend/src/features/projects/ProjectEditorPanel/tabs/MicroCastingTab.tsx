...
export default function MicroCastingTab({
  projectId,
}: MicroCastingTabProps): React.JSX.Element | null {
  const {
    program,
    voiceLines,
    pieces,
  ...
  if (program.length === 0) {
    return (
      <GlassCard
        variant="solid"
        className="p-12 text-center text-stone-500 border border-dashed border-stone-300/50 flex flex-col justify-center items-center"
      >
        <ListOrdered
          size={32}
          className="mb-4 opacity-30 text-stone-400"
          aria-hidden="true"
        />
        <span className="text-[10px] font-bold antialiased uppercase tracking-widest text-stone-500 mb-2">
          Brak utworów
        </span>
        <span className="text-xs max-w-sm leading-relaxed">
          Zbuduj strukturę koncertu w module "Setlista", zanim przejdziesz do
          podziału ról (divisi).
        </span>
      </GlassCard>
    );
  ...
  const currentPiece = pieces.find(
    (p) => String(p.id) === String(selectedPieceId),
  );
  const currentReferenceRecording = currentPiece
    ? getPrimaryReferenceRecording(currentPiece)
    : null;
  ...
  const unassignedParticipations = projectParticipations.filter(
    (p) => !localCastings.some((c) => String(c.participation) === String(p.id)),
  );
  ...
  const renderHealthCheckBanner = () => {
    ...
    requirements.forEach((req) => {
      const assigned = localCastings.filter(
        (c) => c.voice_line === req.voice_line,
      ).length;
      ...
      return (
        <div className="mb-6 p-4 bg-red-50 border border-red-200/80 rounded-xl flex items-center gap-3 text-red-700 text-xs shadow-sm">
          ...
          <span className="leading-relaxed">
            Wykryto braki w partiach! Zrekrutuj{" "}
            <strong>{missingTotal} osób</strong>, by spełnić wymogi kompozytora.
          </span>
        </div>
      );
    ...
    return (
      <div className="mb-6 p-4 bg-emerald-50 border border-emerald-200/80 rounded-xl flex items-center gap-3 text-emerald-700 text-xs shadow-sm">
        ...
        />
        <span className="leading-relaxed">
          Obsada zoptymalizowana! Wszystkie zapotrzebowania wokalne zostały
          spełnione.
        </span>
      </div>
    );
  ...
  const renderBucket = (
    ...
    const artistsInLine = localCastings.filter(
      (c) => c.voice_line === voiceValue,
    );
    ...
    return (
      <div
        key={voiceValue}
        className={`border rounded-2xl p-4 min-h-[140px] flex flex-col transition-colors ${bucketStyle}`}
      >
        <div className="flex justify-between items-start mb-3 border-b pb-3 border-stone-200/50">
          <div className="flex flex-col gap-1">
            <span
              className={`text-[9px] font-bold antialiased uppercase tracking-widest ${titleStyle}`}
            >
              {voiceLabel}
            </span>
            {targetQuantity !== null && (
              <span
                className={`text-[8px] uppercase tracking-widest font-bold antialiased ${assignedCount < targetQuantity ? "text-red-500" : "text-emerald-600"}`}
              >
          ...
          <span
            className={`text-[9px] font-bold antialiased px-2 py-1 rounded-md flex items-center gap-1.5 border shadow-sm ${countStyle}`}
          >
            <Users size={10} aria-hidden="true" /> {assignedCount}{" "}
        ...
        <DroppableBucket
          id={voiceValue}
          isDeficit={targetQuantity !== null && assignedCount < targetQuantity}
          targetQuantity={targetQuantity}
        >
          {artistsInLine.length === 0 && (
            <div
              className={`text-[9px] uppercase tracking-widest font-bold antialiased text-center mt-4 pointer-events-none ${targetQuantity !== null && assignedCount < targetQuantity ? "text-red-400" : "text-stone-300"}`}
            >
              Przeciągnij tutaj
            </div>
          )}
      ...
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide border-b border-stone-200/60">
          {program.map((item, idx) => {
            ...
            if (status === "FREE") {
              tabStyle = isSelected
                ? "border-stone-400 text-stone-800 bg-stone-200/50"
                : "border-transparent text-stone-400 hover:text-stone-600 hover:bg-stone-50/50";
            } else if (status === "OK") {
              tabStyle = isSelected
                ? "border-emerald-500 text-emerald-800 bg-emerald-100/50"
                : "border-transparent text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50";
            } else {
              // DEFICIT
              tabStyle = isSelected
                ? "border-red-500 text-red-800 bg-red-100/50"
                : "border-transparent text-red-500 hover:text-red-700 hover:bg-red-50";
            }
        ...
        <GlassCard variant="solid" className="p-6 md:p-8">
          {/* Metadata Banner */}
          {currentPiece &&
            (currentReferenceRecording || (currentPiece as any).voicing) && (
              <div className="flex flex-wrap items-center gap-3 mb-6 p-4 bg-stone-50 border border-stone-200/80 rounded-xl shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
                {(currentPiece as any).voicing && (
                  <span className="flex items-center gap-1.5 text-[9px] font-bold antialiased uppercase tracking-widest text-[#002395] bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
                    <MicVocal size={14} aria-hidden="true" /> Oryginalna Obsada:{" "}
                    {(currentPiece as any).voicing}
                  </span>
                )}
                {currentReferenceRecording && (
                  ...
                  >
                    <PlayCircleIcon size={14} aria-hidden="true" /> Posłuchaj
                    Referencji
                  </a>
                )}
              </div>
