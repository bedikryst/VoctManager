  ...
  if (enrichedCast.length === 0 && enrichedCrew.length === 0) {
    return (
      <div className="text-center py-20 bg-white/40 rounded-[2rem] border border-dashed border-stone-300/60">
        <Banknote
          size={48}
          className="mx-auto mb-4 opacity-30 text-stone-400"
          aria-hidden="true"
        />
        <span className="text-[11px] font-bold antialiased uppercase tracking-widest text-stone-500 block mb-2">
          Brak personelu do wyceny
        </span>
        <span className="text-xs text-stone-400 max-w-sm mx-auto block leading-relaxed">
          Dodaj najpierw wykonawców w zakładkach "Obsada" lub "Ekipa
          Techniczna", aby móc przydzielać stawki.
        </span>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        >
          ...
          <p className="text-[9px] font-bold antialiased uppercase tracking-widest text-blue-300 mb-1.5 relative z-10 flex items-center gap-1.5">
            <Sparkles size={12} aria-hidden="true" /> Budżet Produkcji
          </p>
          <p className="text-3xl font-black tracking-tight relative z-10">
            {kpi.grandTotal.toLocaleString("pl-PL")} PLN
          </p>
        </GlassCard>
        <GlassCard className="p-6 flex flex-col justify-center">
          <p className="text-[9px] font-bold antialiased uppercase tracking-widest text-[#002395]/70 mb-1.5 flex items-center gap-1.5">
            <Users size={12} aria-hidden="true" /> Koszty Artystyczne
          </p>
          <p className="text-2xl font-black text-[#002395] tracking-tight">
            {kpi.castTotal.toLocaleString("pl-PL")} PLN
          </p>
        </GlassCard>
        <GlassCard className="p-6 flex flex-col justify-center relative overflow-hidden">
          <p
            className={`text-[9px] font-bold antialiased uppercase tracking-widest mb-1.5 flex items-center gap-1.5 ${kpi.missingCount > 0 ? "text-orange-600" : "text-stone-400"}`}
          >
            {kpi.missingCount > 0 ? (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-white/60 backdrop-blur-xl border border-stone-200/80 rounded-[2rem] overflow-hidden shadow-sm flex flex-col">
          <div className="p-5 border-b border-stone-200/60 bg-stone-50/50 flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl bg-blue-50 text-[#002395] flex items-center justify-center border border-blue-100 shadow-sm">
            ...
            <table className="w-full text-left border-collapse min-w-[400px]">
              <tbody className="divide-y divide-stone-100/80">
                {enrichedCast.map((part) => {
                  ...
                  const currentFeeValue = isCellDirty
                    ? dirtyFees[String(part.id)].value
                    : String(part.fee || "");

                  return (
                    <tr
                      key={part.id}
                      className={`transition-colors ${isCellDirty ? "bg-orange-50/30" : "hover:bg-white/50"}`}
                    >
                      <td className="p-4">
                        <p className="font-bold text-stone-900 text-sm tracking-tight">
                          {person.first_name} {person.last_name}
                        </p>
                        <p className="text-[9px] font-bold antialiased uppercase tracking-widest text-stone-400 mt-0.5">
                          {person.voice_type_display || person.voice_type}
                        </p>
                      </td>
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2.5">
                          ...
                          />
                          <span
                            className={`text-[10px] font-bold antialiased uppercase tracking-widest ${isCellDirty ? "text-orange-500" : "text-stone-400"}`}
                          >
                            PLN
                          </span>
                        </div>
        ...
        <div className="bg-white/60 backdrop-blur-xl border border-stone-200/80 rounded-[2rem] overflow-hidden shadow-sm flex flex-col">
          <div className="p-5 border-b border-stone-200/60 bg-stone-50/50 flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl bg-stone-100 text-stone-600 flex items-center justify-center border border-stone-200 shadow-sm">
              ...
              <table className="w-full text-left border-collapse min-w-[400px]">
                <tbody className="divide-y divide-stone-100/80">
                  {enrichedCrew.map((assign) => {
                    ...
                    const isCellDirty =
                      dirtyFees[String(assign.id)] !== undefined;
                    const currentFeeValue = isCellDirty
                      ? dirtyFees[String(assign.id)].value
                      : String(assign.fee || "");
                    ...
                    return (
                      ...
                      >
                        <td className="p-4">
                          ...
                          <p className="text-[9px] font-bold antialiased uppercase tracking-widest text-stone-400 mt-0.5">
                            {assign.role_description || person.specialty}
                          </p>
                        </td>
                        <td className="p-4 text-right">
                          <div className="flex items-center justify-end gap-2.5">
        ...
        {isDirty && (
          ...
          >
            ...
            <Button
              ...
              isLoading={isSaving}
              leftIcon={
                !isSaving ? <Save size={14} aria-hidden="true" /> : undefined
              }
            >
              Zapisz do bazy
            </Button>
