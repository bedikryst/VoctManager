...
export default function BudgetWidget({ project, onEdit }: BudgetWidgetProps): React.JSX.Element | null {
    // Strictly consuming pre-fetched arrays from the React Query cache.
    // No external API hooks are invoked over the network here.
    const { participations, crewAssignments } = useProjectData(String(project.id));
