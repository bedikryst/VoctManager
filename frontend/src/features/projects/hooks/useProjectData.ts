    ...
    const results = useQueries({
        queries: [
            ...
            { 
                ...
                queryFn: async () => (await api.get<Rehearsal[]>(`/api/rehearsals/?project=${projectId}`)).data,
                staleTime: 1000 * 60 * 5,
                enabled: !!projectId
            },
            { 
                queryKey: queryKeys.crewAssignments.byProject(projectId!), 
                queryFn: async () => (await api.get<CrewAssignment[]>(`/api/crew-assignments/?project=${projectId}`)).data,
                staleTime: 1000 * 60 * 5,
                enabled: !!projectId
            },
            { 
