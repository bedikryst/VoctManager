/**
 * @file LocationDossier.tsx
 * @description Read-first drawer for a venue (opens on marker/row select).
 * Surfaces what a conductor actually needs at a glance: live local time, the
 * events booked here, the internal "gate code" whisper, and one-tap Maps /
 * directions deep-links. Edit is one click deeper — mirrors the artist dossier.
 * @architecture Enterprise SaaS 2026
 * @module features/logistics/components/LocationDossier
 */
...
const DossierEventRow = ({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-ethereal-ink/8 bg-ethereal-alabaster/70 px-3 py-2.5">
      ...
      <span
        className={cn("h-1.5 w-1.5 shrink-0 rounded-full", imminence.pulse && "animate-pulse")}
...
export function LocationDossier({
  ...
  onEdit,
  onArchive,
}: LocationDossierProps): React.ReactPortal {
  const { t } = useTranslation();
  ...
  const option = location ? getLocationCategoryOption(t, location.category) : null;
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && location && (
        <React.Fragment key="dossier-wrapper">
          <motion.div
          ...
          <motion.aside
            ...
            className="fixed inset-y-0 right-0 z-focus-trap flex w-full max-w-md flex-col border-l border-ethereal-ink/8 bg-ethereal-alabaster shadow-glass-solid"
            role="dialog"
            aria-modal="true"
            aria-label={location.name}
          >
            <header className="flex shrink-0 items-start justify-between gap-4 border-b border-ethereal-ink/6 bg-ethereal-marble/60 px-6 py-5">
              <div className="min-w-0 space-y-2">
                <Eyebrow color="muted">
                  {t("logistics.dossier.eyebrow", "Karta lokacji")}
                </Eyebrow>
                <Heading as="h2" size="xl" truncate className="text-ethereal-ink">
                  {location.name}
                </Heading>
                {option && (
              ...
              <button
                type="button"
                onClick={onClose}
                aria-label={t("logistics.dossier.close", "Zamknij")}
                className="shrink-0 rounded-xl border border-ethereal-ink/8 bg-ethereal-marble/70 p-2.5 text-ethereal-graphite transition-colors hover:text-ethereal-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ethereal-gold/40"
              >
            ...
            <div className="flex-1 space-y-5 overflow-y-auto overflow-x-hidden px-6 py-6">
              ...
              <div className="flex items-center justify-between gap-3 rounded-2xl border border-ethereal-ink/6 bg-ethereal-marble/50 px-4 py-3">
                ...
                <Text as="span" size="lg" weight="semibold" className="tabular-nums text-ethereal-ink">
              ...
              <section className="space-y-3">
                ...
                {upcoming.length > 0 ? (
                  <div className="space-y-2">
                    {upcoming.map((event) => (
                      <DossierEventRow key={event.id} event={event} />
                    ))}
                  </div>
                ) : (
            ...
            <footer className="flex shrink-0 gap-3 border-t border-ethereal-ink/6 bg-ethereal-marble/40 px-6 py-4">
              <Button
                variant="outline"
