          ...
          {filteredGroups.length > 0 ? (
            filteredGroups.map((group) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex items-center gap-3 border-b border-stone-200/60 pb-3 ml-2 ${isArchived ? "opacity-70" : ""}`}
                  >
                    <Briefcase
                      size={18}
                      className={
                        ...
                        >
                          ...
                          >
                            <div className="flex items-start sm:items-center gap-4">
                              ...
                              <div>
                                <h3
                                  className="text-xl font-bold text-stone-900 tracking-tight leading-tight"
                                  style={{ fontFamily: "'Cormorant', serif" }}
                                >
                                  {piece.title}
                                </h3>
                                      ...
                                      {piece.myCasting && (
                                        <div className="bg-blue-50/50 border border-blue-100/50 p-5 rounded-2xl shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)] h-full">
                                          ...
                                          {piece.myCasting.notes ? (
                                            <p className="text-xs text-stone-600 italic leading-relaxed bg-white/60 p-3 rounded-lg border border-blue-50">
                                              "{piece.myCasting.notes}"
                                            </p>
                                          ) : (
                                      ...
                                      <div className="bg-white/50 backdrop-blur-sm border border-stone-200/80 p-5 rounded-2xl shadow-sm h-full">
                                        ...
                                        {piece.allCastings.length > 0 ? (
                                          <div className="grid grid-cols-2 gap-x-4 gap-y-3 max-h-[160px] overflow-y-auto pr-2 scrollbar-hide">
                                            {Object.entries(divisiGroups).map(
                                              ([vl, groupCastings]) => (
                                                <div
                                                  key={vl}
                                                  className="space-y-1.5"
                                                >
                                                  <h5 className="text-[9px] font-bold text-stone-400 uppercase tracking-widest">
                                                    {vl}
                                                  </h5>
                                        ...
                                        <AnimatePresence>
                                          {showLyricsFor === piece.id && (
                                            <motion.div
                                              ...
                                              animate={{ height: "auto" }}
                                              exit={{ height: 0 }}
                                              className="overflow-hidden border-t border-stone-100/50"
                                            >
