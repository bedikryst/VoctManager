...
import type { EnrichedPiece, ProjectMaterialGroup } from '../types/materials.dto';
...
export const useMaterialsData = (userId?: string | number, searchQuery: string = '') => {
    ...
    const groupedMaterials = useMemo<ProjectMaterialGroup[]>(() => {
        ...
        const activeParticipations = myParticipations.filter((participation) => participation.status !== 'DEC');
        const myProjectIds = activeParticipations.map((participation) => String(participation.project));
        const myProjects = projects.filter(
            (project) => myProjectIds.includes(String(project.id)) && project.status !== 'CANC',
        );
        ...
        const groups = myProjects.map((project) => {
            ...
            const projectProgram = programItems
                .filter((programItem) => String(programItem.project) === String(project.id))
                .sort((left, right) => left.order - right.order);

            const enrichedPieces = projectProgram
                .map((programItem) => {
                    const piece = pieces.find((candidate) => String(candidate.id) === String(programItem.piece));

                    if (!piece) {
                        return null;
                    }

                    const composerData = composers.find(
                        (composer) => String(composer.id) === String(piece.composer),
                    ) || null;

                    const allCastingsForPiece = pieceCastings.filter((casting) => {
                        return String(casting.piece) === String(piece.id) &&
                            String((casting as typeof casting & { project_id?: string | number }).project_id) === String(project.id);
                    });

                    const myCasting = allCastingsForPiece.find(
                        (casting) => String(casting.participation) === String(participation.id),
                    ) || null;

                    let pieceTracks = tracks.filter((track) => String(track.piece) === String(piece.id));
        ...
        return groups.sort((left, right) => {
            ...
            return new Date(left.project.date_time).getTime() - new Date(right.project.date_time).getTime();
        });
    }, [composers, myParticipations, pieceCastings, pieces, programItems, projects, tracks, userId]);
        ...
        return groupedMaterials
            .map((group) => {
                const filteredPieces = group.pieces.filter((piece) => {
                    return piece.title.toLowerCase().includes(term) ||
