    ...
    const results = useQueries({
        queries: [
            { 
                ...
                queryFn: async () => (await api.get<Participation[]>(`/api/participations/?artist=${artistId}`)).data, 
                enabled: !!artistId 
            },
            { 
                queryKey: queryKeys.rehearsals.byArtist(artistId!), 
                queryFn: async () => (await api.get<EnrichedRehearsal[]>(`/api/rehearsals/?invited_participations__artist=${artistId}`)).data, 
                enabled: !!artistId 
            },
            { 
                queryKey: queryKeys.projects.active, 
                queryFn: async () => (await api.get<Project[]>('/api/projects/?status=ACTIVE')).data 
            }
        ]
    });
