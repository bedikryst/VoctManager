...
const STYLE_TEXTAREA = "w-full px-4 py-3 text-sm text-stone-800 bg-white/50 backdrop-blur-sm border border-stone-200/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#002395]/20 focus:border-[#002395]/40 transition-all shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)] resize-none font-medium text-xs leading-relaxed";
...
export default function PieceDetailsForm({ 
    ...
    } = usePieceForm(piece, composers, initialSearchContext, onDirtyStateChange, onSuccess);
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <Input 
                            ...
                            placeholder="np. Lacrimosa" disabled={isSubmitting} className="text-lg font-medium"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {isAddingComposer ? (
                                <div className="flex flex-col gap-3 bg-white/50 backdrop-blur-sm p-5 border border-stone-200/80 rounded-xl shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
                                    <div className="grid grid-cols-2 gap-3">
                                        <Input type="text" placeholder="Imię" value={newComposerData.first_name} onChange={e => setNewComposerData({...newComposerData, first_name: e.target.value})} disabled={isSubmitting} />
                                        <Input type="text" placeholder="Nazwisko *" required value={newComposerData.last_name} onChange={e => setNewComposerData({...newComposerData, last_name: e.target.value})} disabled={isSubmitting} />
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <Input type="number" placeholder="Rok ur." value={newComposerData.birth_year} onChange={e => setNewComposerData({...newComposerData, birth_year: e.target.value})} disabled={isSubmitting} />
                                        <Input type="number" placeholder="Rok śm." value={newComposerData.death_year} onChange={e => setNewComposerData({...newComposerData, death_year: e.target.value})} disabled={isSubmitting} />
                                    </div>
                                </div>
                            ) : (
                                <div className="relative">
                                    <Input 
                                        ...
                                        disabled={isSubmitting} className="font-medium"
                                    />
                                    <AnimatePresence>
                                        {isCompDropdownOpen && (
                                            <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="absolute z-50 w-full mt-2 bg-white/90 backdrop-blur-xl border border-stone-200/60 rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.1)] max-h-48 overflow-y-auto overflow-hidden">
                                                ...
                                                {filteredComposers.map(c => (
                                                    <div key={c.id} onMouseDown={() => { setFormData(prev => ({...prev, composer: String(c.id)})); setIsCompDropdownOpen(false); }} className="px-4 py-3 text-sm font-medium text-stone-800 hover:bg-[#002395] hover:text-white cursor-pointer transition-colors">
                        ...
                        <div>
                            ...
                            <Input type="text" value={formData.arranger} onChange={e => setFormData({...formData, arranger: e.target.value})} placeholder="np. John Rutter" disabled={isSubmitting} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div className="md:col-span-2">
                            ...
                            <select value={formData.epoch} onChange={e => setFormData({...formData, epoch: e.target.value})} className={`${STYLE_SELECT} font-medium appearance-none`} disabled={isSubmitting}>
                ...
                <div className="border-t border-stone-200/60 pt-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <Input type="text" value={formData.voicing} onChange={e => setFormData({...formData, voicing: e.target.value})} placeholder="np. SSAATTBB, Chór + Soliści" disabled={isSubmitting} />
                        </div>
                        <div>
                                ...
                                <div key={idx} className="flex justify-between items-center bg-white/80 border border-stone-200/60 px-5 py-3 rounded-xl shadow-sm transition-colors">
                                    ...
                                    <div className="flex items-center gap-5">
                                        <div className="flex items-center gap-2 bg-stone-50 border border-stone-200/80 rounded-lg shadow-inner px-1 py-1">
                                            ...
                                            <button type="button" onClick={() => handleRequirementChange(idx, 1)} disabled={isSubmitting} className="p-2 text-stone-400 hover:text-stone-800 transition-colors active:scale-95 bg-white rounded-md shadow-sm"><Plus size={12}/></button>
                                        </div>
                                        <button type="button" onClick={() => {
                                            const newReqs = [...requirements];
                                            newReqs.splice(idx, 1);
                                            setRequirements(newReqs);
                                        }} disabled={isSubmitting} className="text-stone-300 hover:text-red-500 p-2.5 rounded-lg hover:bg-red-50 transition-colors border border-transparent hover:border-red-100 active:scale-95"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                ...
                <div className="border-t border-stone-200/60 pt-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea value={formData.lyrics_original} onChange={e => setFormData({...formData, lyrics_original: e.target.value})} rows={5} className={STYLE_TEXTAREA} placeholder="Wklej oryginalny tekst utworu..." disabled={isSubmitting} />
                        </div>
                        <div>
                            <label className={STYLE_LABEL}>Tłumaczenie (Notatki)</label>
                            <textarea value={formData.lyrics_translation} onChange={e => setFormData({...formData, lyrics_translation: e.target.value})} rows={5} className={STYLE_TEXTAREA} placeholder="Wklej polskie tłumaczenie..." disabled={isSubmitting} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <Input type="url" value={formData.reference_recording_youtube} onChange={e => setFormData({...formData, reference_recording_youtube: e.target.value})} placeholder="https://youtube.com/watch?v=..." disabled={isSubmitting} />
                        </div>
                        <div>
                            ...
                            <Input type="url" value={formData.reference_recording_spotify} onChange={e => setFormData({...formData, reference_recording_spotify: e.target.value})} placeholder="https://open.spotify.com/track/..." disabled={isSubmitting} />
                        </div>
                    </div>
