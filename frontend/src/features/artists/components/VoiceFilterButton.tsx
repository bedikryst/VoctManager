...
const VOICE_VARIANTS: Record<VoiceType, { active: string; inactive: string; text: string; count: string }> = {
    'S': {
...
export const VoiceFilterButton: React.FC<VoiceFilterButtonProps> = ({ 
    ...
    const baseClasses = "px-5 py-2.5 rounded-2xl flex flex-col items-center min-w-[80px] transition-all active:scale-95 cursor-pointer shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]";
    const borderClasses = voiceType === 'ALL' ? "ml-2 border-l border-stone-200/50" : "border";
    ...
    return (
        ...
        >
            <span className={`text-[9px] font-bold antialiased uppercase tracking-widest ${variant.text}`}>
                {label}
            </span>
            <span className={`text-xl font-black leading-none mt-1.5 ${variant.count}`}>
                {count}
            </span>
        </button>
    );
