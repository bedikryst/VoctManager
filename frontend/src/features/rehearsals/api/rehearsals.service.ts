...
import type { Artist, Attendance, Participation, Project, Rehearsal } from '../../../shared/types';
...
export const RehearsalsService = {
    ...
    updateAttendance: async (id: string, data: AttendanceUpsertDTO): Promise<Attendance> => {
        const response = await api.patch<Attendance>(`/api/attendances/${id}/`, data);
        return response.data;
    },
