 ...
 * rehearsals (the plans a fill can carry over from). Read under the same keys
 * as the project hub's own queries so the two share one cache — but without
 * suspense: the editor is a band inside a card that is already on screen, and
 * it gates itself on `isLoading` rather than unmounting its host.
