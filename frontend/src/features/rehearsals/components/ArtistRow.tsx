...
export const ArtistRow = React.memo(({ part, artist, rehearsalId, existingRecord }: ArtistRowProps) => {
    const [status, setStatus] = useState<AttendanceStatus | null>(existingRecord?.status || null);
    const [minutesLate, setMinutesLate] = useState(existingRecord?.minutes_late ? String(existingRecord.minutes_late) : '');
    ...
    const isSyncing = upsertAttendanceMutation.isPending || deleteAttendanceMutation.isPending;
        ...
        try {
            ...
            await upsertAttendanceMutation.mutateAsync({
                ...
                data: {
                    ...
                    status: nextStatus,
                    minutes_late: nextStatus === 'LATE' && nextMinutesLate ? parseInt(nextMinutesLate, 10) : null,
                    excuse_note:
                        nextStatus === 'EXCUSED' || nextStatus === 'ABSENT' || nextStatus === 'LATE'
                            ? nextNote
                            : null,
                },
            });
