 ...
 * BUGFIX: Implemented proper Blob-based asynchronous download for the Call-Sheet PDF.
 * UX: Highlights current user in casting, exposes Call Time & Dress Code at top level.
...
import { 
  ...
  Shirt, Download, ListOrdered, Users, Loader2, Music,
...
export default function TimelineProjectCard({ event, isExpanded, onToggle, artistId }: TimelineProjectCardProps) {
  ...
  // --- ONDEMAND FETCHING ---
  // Pobieramy program tylko gdy użytkownik wejdzie w zakładkę Repertuar
  const { data: programData, isLoading: isProgramLoading } = useQuery({
    queryKey: ['proj-program', proj.id],
  ...
  // Pobieramy obsadę utworu tylko gdy użytkownik go rozwinie
  const { data: castingsData, isLoading: isCastingsLoading } = useQuery({
    queryKey: ['proj-castings', expandedPieceId, proj.id],
  ...
  const handleDownloadCallSheet = async (e: React.MouseEvent) => {
      ...
      try {
          // Pobieramy plik binarny (PDF) z backendu
          const response = await api.get(`/api/projects/${proj.id}/export_call_sheet/`, { 
              responseType: 'blob' 
          });
          
          // Tworzymy wirtualny link i wymuszamy kliknięcie
          ...
          console.error("[CallSheet Download]", error);
          toast.error("Błąd generowania", { 
              id: toastId, 
              description: "Nie udało się pobrać pliku. Upewnij się, że menedżer wygenerował dokument." 
          });
      } finally {
        ...
        <div className="p-5 md:p-6 lg:p-8 flex flex-col md:flex-row md:items-start justify-between gap-5 cursor-pointer relative z-10 hover:bg-white/5 transition-colors" onClick={onToggle}>
            <div className="flex flex-col sm:flex-row sm:items-start gap-4 md:gap-6">
                ...
                <div>
                    ...
                    {/* ENTERPRISE UX: Glanceability Badges (Always visible) */}
        ...
        <AnimatePresence>
          {isExpanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-white/10 bg-black/20 relative z-0">
              ...
              {/* Internal Tabs Routing */}
              <div className="flex border-b border-white/10 px-5 md:px-8">
                <button onClick={() => setActiveSubTab('LOGISTICS')} className={`py-4 text-[10px] font-bold uppercase tracking-[0.15em] transition-colors border-b-2 mr-6 ${activeSubTab === 'LOGISTICS' ? 'border-blue-400 text-blue-400' : 'border-transparent text-stone-400 hover:text-stone-300'}`}>Logistyka & Plan Dnia</button>
                <button onClick={() => setActiveSubTab('SETLIST')} className={`py-4 text-[10px] font-bold uppercase tracking-[0.15em] transition-colors border-b-2 ${activeSubTab === 'SETLIST' ? 'border-emerald-400 text-emerald-400' : 'border-transparent text-stone-400 hover:text-stone-300'}`}>Repertuar & Divisi</button>
              </div>
                  ...
                  {activeSubTab === 'LOGISTICS' && (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                          <div className="space-y-6">
                              ...
                              {proj.description && (
                                  <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                                      <p className="text-[9px] font-bold uppercase tracking-widest text-stone-400 mb-3 flex items-center gap-2"><AlignLeft size={14}/> Opis wydarzenia</p>
                                      <p className="text-sm text-stone-300 leading-relaxed whitespace-pre-wrap font-serif">{proj.description}</p>
                                  </div>
                              )}
                          </div>
                          
                          {/* Run Sheet & Call Sheet Action */}
                          <div>
                                                                      ...
                                                                      <div key={vl} className="space-y-3">
                                                                          ...
                                                                          <ul className="space-y-2">
                                                                              {groupCastings.map((c: any) => {
                                                                                  // Identyfikujemy bieżącego artystę (zabezpieczenie pól)
