...
import { 
    ...
    ArrowRight, Clock, MapPin, Sparkles, Wrench, 
    ChevronRight, ListOrdered, MicVocal, Plus, Activity, AlertCircle, UserMinus
} from 'lucide-react';
...
export default function AdminDashboard(): React.JSX.Element {
    ...
    const data = useMemo(() => ({
        ...
        rehearsals: extractData(results[1].data) as EnrichedRehearsal[],
        artists: extractData(results[2].data) as Artist[],
        programItems: extractData(results[3].data) as ProgramItem[],
        pieces: extractData(results[4].data) as Piece[]
    }), [results]);

    const adminStats = useMemo(() => {
        const activeProjects = data.projects.filter(p => p.status === 'ACTIVE' || p.status === 'DRAFT').length;
    ...
    const nextProjectStats = useMemo(() => {
        ...
        const rehearsalsLeft = data.rehearsals.filter(r => {
    ...
    // --- NEXT REHEARSAL GLOBAL ALERT ---
    const nextRehearsal = useMemo(() => {
        const now = new Date();
        // Bufor -2 godziny, by trwająca próba od razu nie znikała
        const threshold = new Date(now.getTime() - 2 * 60 * 60 * 1000); 

        const futureRehearsals = data.rehearsals.filter(r => {
        ...
        if (futureRehearsals.length > 0) {
            ...
            const project = data.projects.find(p => String(p.id) === String(next.project));
            return { ...next, projectTitle: project?.title || 'Nieznany projekt' };
        }
        return null;
    }, [data.rehearsals, data.projects]);
    ...
    return (
        <div className="space-y-10 animate-fade-in relative cursor-default pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="col-span-1 bg-[#0a0a0a] rounded-[2rem] p-8 md:p-10 relative overflow-hidden group shadow-[0_20px_40px_rgba(0,0,0,0.3)] shadow-[inset_0_1px_0_rgba(255,255,255,0.15)] text-white border border-stone-800 flex flex-col justify-between">
                ...
                <div className="xl:col-span-2 flex flex-col gap-6">
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className={`${STYLE_PREMIUM_GLASS} flex-1 p-8 md:p-10 flex flex-col justify-between`}>
                    ...
                    {/* 3. NEW FEATURE: NEXT REHEARSAL ALERT */}
                    {nextRehearsal && (
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-gradient-to-r from-orange-50 to-white border border-orange-200/60 shadow-sm rounded-[2rem] p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden group">
                            ...
                            <div className="relative z-10 flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0 justify-end">
                                {/* ENTERPRISE UX: Absence Warning Pill */}
            ...
            {/* --- NAVIGATION MODULES (The App Grid) --- */}
