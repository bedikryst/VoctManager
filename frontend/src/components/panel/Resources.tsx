...
import { 
  FileText, Download, Shirt, BookOpen, 
...
export default function Resources(): React.JSX.Element {
  ...
  const handleAdminAction = () => {
    toast.info("Moduł CMS w przygotowaniu", {
      description: "Możliwość wgrywania i edycji plików na serwerze zostanie aktywowana po podpięciu docelowego API."
    });
  };
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5">
                  <div>
                      <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/80 backdrop-blur-md border border-white/60 shadow-sm mb-4">
                          <Archive size={12} className="text-[#002395]" aria-hidden="true" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {DOCUMENT_CATEGORIES.map((category, index) => (
              ...
              >
                  ...
                  <div className={`absolute -right-8 -top-8 opacity-[0.03] pointer-events-none transition-transform duration-700 ${category.iconColor}`} aria-hidden="true">
                      {React.isValidElement(category.icon) && React.cloneElement(category.icon as React.ReactElement, { size: 160, strokeWidth: 1 } as any)}
                  </div>
                      ...
                      <div className="flex justify-between items-start mb-4">
                          ...
                          {isAdmin && (
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button onClick={handleAdminAction} className="p-2 text-stone-400 hover:text-[#002395] hover:bg-blue-50 rounded-lg transition-colors" aria-label="Edytuj kategorię">
                                      <Edit2 size={14} aria-hidden="true" />
                                  </button>
                                  <button onClick={handleAdminAction} className="p-2 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" aria-label="Usuń kategorię">
                          ...
                          {category.files.map((file) => (
                              ...
                              >
                                  <div className="flex items-start gap-4 overflow-hidden pr-4 flex-1">
                                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)] transition-colors flex-shrink-0 ${category.bgColor} ${category.borderColor} ${category.iconColor} group-hover/file:bg-[#002395] group-hover/file:border-[#001766] group-hover/file:text-white`}>
                                  ...
                                  <div className="flex items-center gap-2 self-center flex-shrink-0">
                                      ...
                                      {isAdmin && (
                                          <button 
                                              onClick={(e) => { e.stopPropagation(); handleAdminAction(); }} 
