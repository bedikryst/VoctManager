 ...
 * @architecture
 * ENTERPRISE 2026: 
 * - Implements strict Keyboard Accessibility (ESC to close, Focus Management).
 * - "Fluid Tab Routing": Uses AnimatePresence for cinematic cross-fades between module tabs.
 * - Responsive Dimensionality: Replaces full-width bounds with dynamic Viewport constraints (max-w-[1400px]).
 * - Unified Swipeable Mobile Navigation replaces clunky legacy dropdowns.
...
export default function ProjectEditorPanel({ 
  ...
  // ENTERPRISE FIX: Global Keyboard Shortcuts (Escape to close)
  useEffect(() => {
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          {/* 1. Backdrop Overlay (Darker & blurrier for focus) */}
          <motion.div 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
          ...
          {/* 2. Slide-out Panel (Bounded width, independent scroll) */}
          <motion.div 
          ...
          >
            ...
            <div className="flex-shrink-0 bg-[#f8f7f4]/95 backdrop-blur-2xl z-20 px-6 md:px-10 pt-8 pb-4">
              ...
              {/* Navigation Tabs (Unified Swipeable Area for both Desktop & Mobile) */}
              {project && (
            ...
            {/* Content Area (Scrollable with Cross-Fade) */}
            <div className="flex-1 overflow-y-auto overflow-x-hidden relative scrollbar-hide bg-gradient-to-b from-transparent to-stone-50/50">
              <div className="p-4 md:px-10 md:pb-10 pt-2 min-h-full">
                ...
                {/* ENTERPRISE FIX: Cinematic Tab Switching */}
