...
export default function CrewTab({ projectId }: CrewTabProps): React.JSX.Element | null {
  ...
  const { crew, crewAssignments } = useProjectData(projectId);
      ...
      <form onSubmit={handleAssign} className={`${STYLE_GLASS_CARD} p-6 flex flex-col md:flex-row gap-5 items-end`}>
        <div className="flex-1 w-full">
          ...
          <select 
            ...
            className={STYLE_GLASS_INPUT} disabled={isMutating}
          >
        ...
        <div className="flex-1 w-full">
          ...
          <input 
            type="text" value={roleDesc} onChange={(e) => setRoleDesc(e.target.value)} 
            placeholder="np. Akustyk FOH" className={STYLE_GLASS_INPUT} disabled={isMutating}
          />
        </div>
        <button 
          type="submit" disabled={isMutating || !selectedCrewId}
