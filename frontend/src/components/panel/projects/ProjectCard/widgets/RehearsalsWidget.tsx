 ...
 * @architecture
 * Synchronously consumes pre-fetched rehearsal arrays from the parent context.
 * Implements strict timeline sorting via `useMemo`.
 * Displays dynamic `absent_count` injected by the Django backend.
 * @module project/ProjectCard/widgets/RehearsalsWidget
...
// Rozszerzenie typu o dynamiczne pole z backendu bez modyfikowania głównego pliku typów
...
export default function RehearsalsWidget({ project }: RehearsalsWidgetProps): React.JSX.Element | null {
  ...
  const { openPanel, rehearsals: projectRehearsals, participations: projectParticipations } = context;
  ...
  return (
    <div 
      onClick={() => openPanel(project, 'REHEARSALS')} 
        ...
        <div className="flex flex-col h-full">
          <div className="mb-4">
            ...
            <div className="w-full bg-stone-100 rounded-full h-1.5">
              <div className="bg-[#002395] h-1.5 rounded-full transition-all duration-500 ease-out" style={{ width: `${progressPercentage}%` }} />
            </div>
          </div>
