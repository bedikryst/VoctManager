 ...
 * @architecture
 * Implements collocated UI state (`isOpen`) to prevent unnecessary renders in the parent container.
 * Utilizes `useMemo` for sorting the agenda array, ensuring stability during Framer Motion 
 * presence transitions and preventing costly string comparisons on every render.
...
export default function RunSheetWidget({ project }: RunSheetWidgetProps): React.JSX.Element {
  const { openPanel } = useContext(ProjectDataContext) as IProjectDataContext;
    ...
    <div className="bg-white border border-stone-200/80 rounded-2xl shadow-sm overflow-hidden">
      ...
      >
        <h4 className="flex items-center gap-2.5 text-[10px] font-bold antialiased uppercase tracking-widest text-stone-500">
          <ListOrdered size={16} className="text-[#002395]"/> Harmonogram dnia koncertu
        </h4>
            ...
            <div className="p-5 bg-stone-50/30">
              {hasRunSheet ? (
                <div className="relative pl-5 border-l-2 border-stone-200 space-y-5 ml-2">
                  {sortedRunSheet.map((item, idx) => (
                    <div key={item.id || idx} className="relative">
                      <div className="absolute -left-[27px] top-1 w-4 h-4 bg-white border-[3px] border-[#002395] rounded-full shadow-sm" />
                      <div className="flex flex-col gap-1">
                        ...
                        <div>
                          ...
                          {item.description && (
                            <p className="text-xs text-stone-500 italic mt-1 leading-relaxed">{item.description}</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="text-center py-6 bg-white/50 rounded-xl border border-dashed border-stone-200">
                  <ListOrdered size={24} className="mx-auto mb-2 text-stone-300" />
                  <p className="text-xs text-stone-400 font-medium">Brak harmonogramu dostępnego dla tego wydarzenia.</p>
                </div>
              )}
            </div>
          </motion.div>
