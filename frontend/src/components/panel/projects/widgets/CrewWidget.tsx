 ...
 * @architecture
 * Maps crew assignments to collaborator profiles, extracting roles or falling back to specialties.
...
import type { Project, CrewAssignment, Collaborator } from '../../../../types';
...
/**
 * CrewWidget Component
 * @param {CrewWidgetProps} props - Component properties.
...
export default function CrewWidget({ project }: CrewWidgetProps): React.JSX.Element | null {
  ...
  const { crewAssignments, crew, openPanel } = context;

  // --- Derived Data (Memoized) ---
  const projectCrew = useMemo<CrewAssignment[]>(() => {
    return crewAssignments?.filter((c) => c.project === project.id) || [];
  }, [crewAssignments, project.id]);
  ...
  /**
   * Invokes the parent's panel controller to open the Crew configuration tab.
   */
