 ...
 * @architecture
 * Acts as a layout container and orchestrator for domain-specific micro-widgets.
 * Handles top-level project mutations (status change, deletion) and document streaming.
...
// --- Static Constants ---
// Extracted from the render cycle to optimize memory allocation
...
export default function ProjectCard({ project }: ProjectCardProps): React.JSX.Element {
  ...
  const { openPanel, handleDelete, fetchGlobal } = context;
  ...
  /**
   * Toggles the active/done lifecycle status of the project and notifies the user.
  ...
  /**
   * Handles binary data streaming (BLOBs) for document generation with visual loading states.
   * Extracts filename dynamically from the Content-Disposition header.
   * @param {React.MouseEvent<HTMLButtonElement>} e - The click event.
   * @param {string} endpoint - The API endpoint suffix.
   * @param {string} defaultFilename - Fallback filename if header is missing.
   * @param {string} loaderKey - State key to identify the active loading indicator on the button.
   */
