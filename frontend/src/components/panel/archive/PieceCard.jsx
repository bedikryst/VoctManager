 ...
 * ENTERPRISE UPGRADES: High-density data rendering for advanced metadata 
 * (Voicing, Duration, YouTube Links, and Original/Translated Lyrics).
 * UI UPGRADE 2026: Glassmorphism containers, antialiased micro-typography, 
 * and fluid spring-based layout expansions.
...
import { Edit2, Trash2, Music, FileText, Headphones, ChevronDown, ChevronUp, Download, Clock, Youtube, AlignLeft } from 'lucide-react';

export default function PieceCard({ piece, isExpanded, onToggleExpand, onOpenPanel, onDelete, getComposerInfo }) {
  const comp = getComposerInfo(piece.composer);
  const [showLyrics, setShowLyrics] = useState(false);

  const handleAudioPlay = (e) => {
    document.querySelectorAll('audio').forEach(audioEl => { if (audioEl !== e.target) audioEl.pause(); });
  };
  ...
  /**
   * Formats total seconds into a readable string (e.g., "4 min 30 sek")
   */
  ...
  const glassCardStyle = "bg-white/70 backdrop-blur-xl border border-white/60 shadow-[0_4px_20px_rgb(0,0,0,0.03)] shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] rounded-2xl overflow-hidden transition-all duration-300";

  return (
    <div className={`${glassCardStyle} ${isExpanded ? 'border-[#002395]/30 shadow-[0_10px_30px_rgba(0,35,149,0.05)]' : 'hover:border-[#002395]/20 hover:-translate-y-0.5 hover:shadow-[0_12px_24px_rgb(0,0,0,0.06)]'}`}>
      <div className="p-4 md:p-5 flex flex-col md:flex-row md:items-center justify-between gap-5 cursor-pointer hover:bg-white/40 transition-colors relative z-10" onClick={() => onToggleExpand(piece.id)}>
        ...
        <div className="flex items-start gap-4 md:gap-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-[10px] font-bold antialiased text-stone-500 uppercase tracking-widest mt-1">
              ...
              {piece.composition_year && (
                  <span className="flex items-center gap-1.5 border-l border-stone-300 pl-3">
                      <Clock size={12}/> {piece.composition_year}
                  </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-2.5 text-[9px] font-bold antialiased uppercase tracking-widest text-stone-400">
                ...
                {piece.estimated_duration && <span className="px-2 py-1 border border-stone-200/80 bg-white/60 backdrop-blur-sm rounded-md flex items-center gap-1 shadow-sm"><Clock size={10}/> {formatDuration(piece.estimated_duration)}</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          <div className="hidden md:flex gap-2">
            {piece.sheet_music && <span className="px-2.5 py-1.5 bg-blue-50 text-[#002395] text-[9px] tracking-widest font-bold antialiased uppercase rounded-lg border border-blue-100 shadow-sm flex items-center gap-1.5"><FileText size={10}/> PDF</span>}
            {piece.tracks?.length > 0 && <span className="px-2.5 py-1.5 bg-emerald-50 text-emerald-700 text-[9px] tracking-widest font-bold antialiased uppercase rounded-lg border border-emerald-100 shadow-sm flex items-center gap-1.5"><Headphones size={10}/> Audio ({piece.tracks.length})</span>}
          </div>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-stone-50/40 border-t border-white/60 overflow-hidden relative z-0">
            <div className="p-6 md:p-8">
              ...
              <div className="flex flex-col lg:flex-row justify-between gap-8 mb-10">
                        ...
                        <div className="mt-5">
                            ...
                            <div className="flex flex-wrap gap-2 mt-2.5">
                                ...
                                <span key={req.id} className="px-2.5 py-1 bg-white/60 border border-[#002395]/20 text-[#002395] text-[9px] font-bold antialiased uppercase tracking-widest rounded-md shadow-sm">
                                    {req.voice_line_display}: {req.quantity}
                                </span>
                                ))}
                            </div>
                        </div>
                ...
                <div className="flex flex-col gap-3 min-w-[220px] border-t lg:border-t-0 border-stone-200/60 pt-6 lg:pt-0">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onOpenPanel(piece, 'TRACKS'); }} className="w-full py-3.5 bg-white border border-stone-200/80 text-stone-600 text-[10px] font-bold antialiased uppercase tracking-widest rounded-xl hover:border-emerald-500/40 hover:text-emerald-700 hover:shadow-md transition-all flex items-center justify-center gap-2.5 shadow-sm active:scale-95">
                    <Headphones size={14} /> Zarządzaj Audio
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); onDelete(piece.id); }} className="w-full py-3.5 mt-2 text-stone-400 hover:text-red-600 text-[10px] font-bold antialiased uppercase tracking-widest transition-colors flex items-center justify-center gap-2.5 active:scale-95">
                    <Trash2 size={14} /> Usuń Utwór
                  </button>
                </div>
              </div>
                  ...
                  <div className="border-t border-stone-200/60 pt-8 mb-8 space-y-5">
                      {piece.reference_recording && (
                          <a href={piece.reference_recording} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 text-[10px] font-bold antialiased uppercase tracking-[0.15em] text-red-600 hover:text-red-700 bg-white hover:bg-red-50 px-5 py-3 rounded-xl transition-colors border border-red-100 shadow-sm active:scale-95">
                              <Youtube size={16} /> Nagranie Referencyjne YouTube / Spotify
                          </a>
                      )}
                      ...
                      {(piece.lyrics_original || piece.lyrics_translation) && (
                          <div className="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl shadow-sm overflow-hidden">
                              <button onClick={() => setShowLyrics(!showLyrics)} className="w-full flex items-center justify-between p-5 text-left hover:bg-white/80 transition-colors">
                                  ...
                                  <span className="text-stone-400 bg-white shadow-sm p-1.5 rounded-full border border-stone-100">{showLyrics ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}</span>
                              </button>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 border-t border-stone-200/60 pt-8">
                <div>
                  <h4 className="flex items-center gap-2 text-[10px] font-bold antialiased uppercase tracking-widest text-stone-500 mb-4 ml-1"><FileText size={14} className="text-[#002395]" /> Partytura (PDF)</h4>
                  {piece.sheet_music ? (
                    <a href={piece.sheet_music} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 px-6 py-3.5 bg-white border border-stone-200/80 rounded-xl text-[10px] uppercase tracking-widest font-bold antialiased text-[#002395] hover:border-[#002395]/40 transition-all shadow-sm active:scale-95"><Download size={14} /> Pobierz Nuty</a>
                ...
                <div>
                  <h4 className="flex items-center gap-2 text-[10px] font-bold antialiased uppercase tracking-widest text-stone-500 mb-4 ml-1"><Headphones size={14} className="text-emerald-600" /> Ścieżki Ćwiczeniowe</h4>
                      ...
                      {piece.tracks.map(track => (
                        <div key={track.id} className="bg-white/80 backdrop-blur-sm p-3.5 rounded-xl border border-stone-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <span className="text-[10px] uppercase tracking-widest font-bold antialiased text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100 flex items-center gap-2">
                              <PlayCircle size={12}/> {track.title || track.voice_part_display}
                          </span>
