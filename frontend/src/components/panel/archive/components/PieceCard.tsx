 ...
 * Refactored to consume `EnrichedPiece` (Data Enrichment pattern) directly, 
 * eliminating prop-drilling from the parent controller.
 * @module archive/components/PieceCard
 */
...
import type { EnrichedPiece } from '../ArchiveManagement';
...
const getEpochLabel = (val: string) => EPOCHS.find(e => e.value === val)?.label || val;
      ...
      >
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          <div className="hidden md:flex gap-2">
            ...
            {pieceTracks.length > 0 && <span className="px-2.5 py-1.5 bg-emerald-50 text-emerald-700 text-[9px] tracking-widest font-bold antialiased uppercase rounded-lg border border-emerald-100 shadow-sm flex items-center gap-1.5"><Headphones size={10} /> Audio ({pieceTracks.length})</span>}
          </div>
            ...
            <div className="p-6 md:p-8">
              <div className="flex flex-col lg:flex-row justify-between gap-8 mb-10">
                ...
                <div className="flex flex-col gap-3 min-w-[220px] border-t lg:border-t-0 border-stone-200/60 pt-6 lg:pt-0">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="w-full py-3.5 mt-2 text-stone-400 hover:text-red-600 text-[10px] font-bold antialiased uppercase tracking-widest transition-colors flex items-center justify-center gap-2.5 active:scale-95">
                  ...
                  <div className="border-t border-stone-200/60 pt-8 mb-8">
                      <div className="flex flex-wrap gap-3">
                        {referenceLinks.map(link => (
                          ...
                          >
                            {link.platform === 'youtube' ? <Youtube size={16} /> : <Headphones size={16} />}
                            {link.platform === 'youtube' ? 'Nagranie Referencyjne YouTube' : link.platform === 'spotify' ? 'Nagranie Referencyjne Spotify' : 'Nagranie Referencyjne'}
                          </a>
                        ))}
                      </div>
                  </div>
