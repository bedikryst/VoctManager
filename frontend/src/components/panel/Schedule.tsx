 ...
 * ENTERPRISE UPGRADE 2026: Powered by React Query for parallel fetching and cache invalidation.
 * Replaced native alerts with Sonner toasts. Unified Project and Rehearsal models 
 * into a strictly typed TimelineEvent interface.
...
import { 
  ...
  ShieldAlert, Send, X, Loader2, ChevronDown, ChevronUp, AlignLeft 
} from 'lucide-react';
...
// --- Static Styles ---
const STYLE_GLASS_CARD = "bg-white/70 backdrop-blur-xl border border-white/60 shadow-[0_4px_20px_rgb(0,0,0,0.03)] shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] rounded-2xl relative overflow-hidden";
...
export default function Schedule(): React.JSX.Element {
  ...
  // Modal States
  const [reportModal, setReportModal] = useState<{ isOpen: boolean; event: TimelineEvent | null }>({ isOpen: false, event: null });
  const [reportForm, setReportForm] = useState({ status: 'ABSENT', notes: '' });
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false);

  const [castModal, setCastModal] = useState<{ isOpen: boolean; event: string | null; artists: any[]; isLoading: boolean }>({ isOpen: false, event: null, artists: [], isLoading: false });

  // --- Data Fetching Engine (React Query) ---
  const results = useQueries({
    queries: [
      { queryKey: ['sch-rehearsals'], queryFn: async () => (await api.get('/api/rehearsals/')).data, enabled: !!user?.id },
      { queryKey: ['sch-projects'], queryFn: async () => (await api.get('/api/projects/')).data, enabled: !!user?.id },
      // Zabezpieczenie: pobieramy tylko partycypacje artysty
      { queryKey: ['sch-participations', user?.id], queryFn: async () => (await api.get(`/api/participations/?artist=${user?.id}`)).data, enabled: !!user?.id },
      { queryKey: ['sch-attendances'], queryFn: async () => (await api.get('/api/attendances/')).data, enabled: !!user?.id }
    ]
  });
  ...
  useEffect(() => {
    if (isError) {
      toast.error("Błąd synchronizacji", { description: "Nie udało się załadować kalendarza." });
    }
  }, [isError]);

  // --- Data Aggregation & Timeline Construction ---
  const timelineEvents = useMemo<TimelineEvent[]>(() => {
    if (!user?.id || isLoading) return [];

    const rehearsals = Array.isArray(results[0].data) ? results[0].data as Rehearsal[] : [];
    const projects = Array.isArray(results[1].data) ? results[1].data as Project[] : [];
    const myParticipations = Array.isArray(results[2].data) ? results[2].data as Participation[] : [];
    const attendances = Array.isArray(results[3].data) ? results[3].data as Attendance[] : [];

    const events: TimelineEvent[] = [];

    // Odrzucamy anulowane partycypacje (DEC)
    ...
    rehearsals.forEach(reh => {
      ...
      if (!myPart) return; // Not participating in this project

      // Check if specifically invited or if it's tutti (empty invited array)
    ...
    // 2. Parse Main Projects (Concerts)
    ...
    return events.sort((a, b) => a.date_time.getTime() - b.date_time.getTime());
  }, [results, user, isLoading]);

  // --- Filtering ---
  const filteredEvents = useMemo<TimelineEvent[]>(() => {
    const now = new Date();
    // Żeby uwzględnić wydarzenia z danego dnia jako "Nadchodzące", odejmujemy 24h
    const threshold = new Date(now.getTime() - 24 * 60 * 60 * 1000); 

    return timelineEvents.filter(e => {
      if (viewMode === 'UPCOMING') return e.date_time >= threshold;
      return e.date_time < threshold;
  ...
  const handleReportSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const toastId = toast.loading("Wysyłanie zgłoszenia do inspektora...");

    try {
      const myParticipations = Array.isArray(results[2].data) ? results[2].data as Participation[] : [];
      const myPart = myParticipations.find(p => String(p.project) === String(reportModal.event!.project_id));
      
      if (!myPart) throw new Error("Brak przypisania do projektu.");
      ...
      // Sprawdzamy czy mamy już wpis w Attendances (żeby użyć PATCH lub POST)
      const attendances = Array.isArray(results[3].data) ? results[3].data as Attendance[] : [];
      ...
      toast.success("Twoje zgłoszenie zostało zarejestrowane.", { id: toastId });
    } catch (err) {
      console.error(err);
  ...
  const handleFetchCast = async (project_id: string | number, eventTitle: string) => {
      ...
      try {
          ...
          toast.error("Błąd pobierania obsady.");
          setCastModal(prev => ({ ...prev, isLoading: false }));
      }
  };

  const toggleExpand = (id: string) => setExpandedEventId(prev => prev === id ? null : id);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div className="inline-flex items-center p-1.5 bg-white/60 backdrop-blur-xl border border-white/60 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] rounded-xl overflow-x-auto max-w-full scrollbar-hide">
            {[{ id: 'UPCOMING', label: 'Nadchodzące' }, { id: 'PAST', label: 'Historia' }].map(tab => (
              <button 
                key={tab.id} 
                onClick={() => setViewMode(tab.id as 'UPCOMING' | 'PAST')} 
      ...
      <div className="space-y-4 relative z-10">
        {isLoading ? (
            ...
            {filteredEvents.map((ev) => {
              ...
              return (
                <motion.div 
                  ...
                  className={`${STYLE_GLASS_CARD} transition-all duration-300 ${isExpanded ? 'border-[#002395]/30 shadow-[0_10px_30px_rgba(0,35,149,0.05)]' : 'hover:border-[#002395]/20 hover:-translate-y-0.5 hover:shadow-[0_12px_24px_rgb(0,0,0,0.06)]'}`}
                >
                  ...
                  <div className={`absolute -right-8 -top-8 opacity-[0.02] pointer-events-none transition-transform duration-700 ${isProject ? 'text-[#002395]' : 'text-stone-500'}`}>
                      <IconComp size={150} strokeWidth={1} aria-hidden="true" />
                  </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 md:gap-6">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-2 text-[10px] font-bold antialiased uppercase tracking-widest text-stone-500">
                          <span className="flex items-center gap-1.5"><Clock size={12} aria-hidden="true" /> {ev.date_time.toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit' })}</span>
                          <span className="flex items-center gap-1.5 truncate max-w-[200px]"><MapPin size={12} className="flex-shrink-0" aria-hidden="true" /> <span className="truncate">{ev.location || 'Brak danych'}</span></span>
                        </div>
                      </div>
                    </div>
                        ...
                        >
                            <div className="p-6 md:p-8 space-y-6">
                                ...
                                {isProject ? (
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                {ev.description && (
                                                    <div className="bg-white/60 p-4 rounded-xl border border-stone-100/50 shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
                                                        <p className="text-[9px] font-bold antialiased uppercase tracking-widest text-stone-500 mb-2 flex items-center gap-1.5"><AlignLeft size={12} aria-hidden="true" /> Opis / Notatki</p>
                                    ...
                                    ev.focus && (
                                        <div>
                                            <h4 className="text-[10px] font-bold antialiased uppercase tracking-[0.15em] text-stone-400 mb-3 border-b border-stone-200/60 pb-2 flex items-center gap-1.5"><AlignLeft size={14} aria-hidden="true" /> Notatki Dyrygenta</h4>
                                ...
                                <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-stone-200/60">
                                    ...
                                    {isProject && (
                                        ...
                                        >
                                            <Users size={14} aria-hidden="true" /> Zobacz Obsadę
                                        </button>
                                    )}
                                </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`${STYLE_GLASS_CARD} p-16 flex flex-col items-center justify-center text-center`}>
             <Calendar size={48} className="text-stone-300 mb-4 opacity-50" aria-hidden="true" />
          ...
          >
            ...
            >
              <div className="flex justify-between items-center p-6 md:p-8 border-b border-stone-200/50 bg-stone-50/50">
                ...
                <button onClick={() => setReportModal({ isOpen: false, event: null })} className="text-stone-400 hover:text-stone-900 bg-white border border-stone-200 shadow-sm p-2.5 rounded-xl transition-all active:scale-95"><X size={18} aria-hidden="true" /></button>
              </div>
              ...
              <div className="p-6 md:p-8">
                ...
                <form onSubmit={handleReportSubmit} className="space-y-6">
                  <div>
                    ...
                    <select value={reportForm.status} onChange={e => setReportForm({...reportForm, status: e.target.value})} className={`${STYLE_GLASS_INPUT} font-bold appearance-none`}>
                  ...
                  <div>
                    ...
                    <textarea 
                        ...
                        className={`${STYLE_GLASS_INPUT} resize-none font-medium`}
                    />
                  </div>
                  <div className="pt-4 border-t border-stone-100">
                      ...
                      >
                        {isSubmitting ? <Loader2 size={16} className="animate-spin" aria-hidden="true" /> : <Send size={16} aria-hidden="true" />} Wyślij Zgłoszenie
                      </button>
                  </div>
                </form>
              </div>
            ...
            >
                ...
                >
                    <div className="flex justify-between items-center p-6 md:p-8 border-b border-stone-200/50 bg-white/80 backdrop-blur-xl flex-shrink-0 z-20">
                        ...
                        <button onClick={() => setCastModal({ isOpen: false, event: null, artists: [], isLoading: false })} className="text-stone-400 hover:text-stone-900 bg-white border border-stone-200 shadow-sm p-2.5 rounded-xl transition-all active:scale-95"><X size={18} aria-hidden="true" /></button>
                    </div>

                    <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-stone-50/50">
                        {castModal.isLoading ? (
                            <div className="flex flex-col items-center justify-center py-12">
                                <Loader2 size={32} className="animate-spin text-stone-300 mb-4" aria-hidden="true" />
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {Object.entries(castModal.artists.reduce((acc: any, artist: any) => {
                                    const group = artist.voice_type || 'Inne';
                                    if (!acc[group]) acc[group] = [];
                                    acc[group].push(artist);
                                    return acc;
                                }, {})).map(([group, artists]: [string, any]) => (
