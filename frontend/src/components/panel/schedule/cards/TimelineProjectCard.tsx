...
export default function TimelineProjectCard({ event, isExpanded, onToggle, artistId }: TimelineProjectCardProps): React.JSX.Element {
    ...
    const {
        activeSubTab, setActiveSubTab, expandedPieceId, setExpandedPieceId,
        isDownloading, programItems, isProgramLoading, castings, isCastingsLoading,
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                                {proj.call_time && (
                                    <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-orange-500/20 text-orange-300 border border-orange-500/30">
                                        <Clock size={12} aria-hidden="true" /> Zbiórka: {new Date(proj.call_time).toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit' })}
                                    </span>
                                )}
                                {combinedDressCode && (
                                ...
                                <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-blue-200 truncate max-w-[200px]">
                                    <MapPin size={12} className="flex-shrink-0" aria-hidden="true" /> <span className="truncate">{event.location || 'Brak lok.'}</span>
                                </span>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div className="space-y-6">
                                            {(proj.dress_code_female || proj.dress_code_male) && (
                                                <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                                                    <p className="text-[9px] font-bold uppercase tracking-widest text-stone-400 mb-3 flex items-center gap-2"><Shirt size={14} aria-hidden="true" /> Szczegóły ubioru</p>
                                        ...
                                        <div>
                                            <div className="flex justify-between items-center mb-4">
                                                ...
                                                <Button 
                                                    ...
                                                    leftIcon={!isDownloading ? <Download size={12} aria-hidden="true" /> : undefined}
                                ...
                                {activeSubTab === 'SETLIST' && (
                                    <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
                                        <div className="xl:col-span-2 xl:order-last">
                                            {proj.spotify_playlist_url ? (
                                                ...
                                                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 text-center flex flex-col items-center justify-center h-full min-h-[150px]">
                                                    <Music size={24} className="text-stone-600 mb-2 opacity-50" aria-hidden="true" />
                                        ...
                                        <div className="xl:col-span-3 space-y-4">
                                            {isProgramLoading ? (
                                                <div className="flex justify-center py-10"><Loader2 className="animate-spin text-emerald-400" aria-hidden="true" /></div>
                                                                ...
                                                                <div className="flex items-center gap-4">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <p className="text-xs text-stone-400 flex items-center gap-1.5 mt-0.5">
                                                                            <Users size={12} aria-hidden="true" /> {isPieceExpanded ? 'Ukryj obsadę' : 'Rozwiń obsadę (divisi)'}
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                                {isPieceExpanded ? <ChevronUp size={18} className="text-emerald-500" aria-hidden="true" /> : <ChevronDown size={18} className="text-stone-500" aria-hidden="true" />}
                                                            </div>
                                                                ...
                                                                {isPieceExpanded && (
                                                                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden bg-black/40 border-t border-white/5">
                                                                        <div className="p-4 md:p-6">
                                                                            {isCastingsLoading ? (
                                                                                <div className="flex justify-center py-4"><Loader2 size={16} className="animate-spin text-stone-500" aria-hidden="true" /></div>
                                                                            ) : castings.length > 0 ? (
                                                                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                                                                                    {Object.entries(castings.reduce((acc: Record<string, any[]>, c: any) => {
                                                                                        ...
                                                                                        <div key={vl} className="space-y-3">
                                                                                            ...
                                                                                            <ul className="space-y-2">
                                                                                                {(groupCastings as any[]).map((c: any) => {
