/**
 * @file NativeSelect.tsx
 * @description Styled trigger around a native <select>. The popup is drawn by the
 * OS, so it takes no part in the Ethereal language — which is why `Select` (Radix)
 * exists and is the one to reach for. This remains only for the surfaces not yet
 * migrated; see `docs/project-hub-design-remediation.md` §5. Do not use it in new code.
 * @module shared/ui/primitives/NativeSelect
 */
...
const selectVariants = cva(
  "w-full appearance-none rounded-control text-sm text-ethereal-ink transition-all duration-300 focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50",
  {
    variants: {
      variant: {
        glass:
