...
ANALYZE_SCORE = Prompt(
    ...
    system=(
        ...
        "  - composition_year: ONLY a year this score prints as the date the "
        "    MUSIC WAS WRITTEN — a date under the final bar, a dated dedication "
        "    or manuscript date, 'skomponowano 1998', 'composed 1723' in a "
        "    critical edition's front matter. A reprint's title page dates the "
        "    EDITION: the copyright line (©), publication, printing, edition, "
        "    imprimatur and arrangement years are all WRONG answers here. Do "
        "    NOT fall back on your own knowledge of when a famous work was "
        "    written — this field records what the page states, not what you "
        "    recall. Most scores print no such date: null is the expected "
...
GENERATE_PROGRAM_NOTE = Prompt(
    ...
    system=(
        ...
        "    its original language. Add a parenthetical gloss ONLY when the sung "
        "    language differs from the target language — a quotation the reader "
        "    already understands needs no translation, and a gloss must never be "
        "    in a third language.\n"
