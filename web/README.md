...
* **The press pack is built, not committed.** `npm run press:pack` assembles `public/press/voctensemble-press-YYYY-MM.zip` from `press-pack/manifest.yaml` — the photographs the ensemble cleared and the current technical rider — plus five documents it generates from the site's own data (the three biograms, the credits, the invoicing sheet, the programmes, the recording links). The archive is `.gitignore`d for the same reason the photographs are, so **build it where the originals are and upload it to the build host**, beside `src/assets/photos/`. Its ABSENCE is a supported state: `/press` looks for the file (`lib/pressPack.ts`) and renders "materials on request" wherever it is missing, which is what is true there. The script refuses to build and names what it is waiting for, rather than shipping a pack with a gap.
