...
* **The press pack is built, not committed.** `npm run press:pack` writes `public/press/`: the press photographs cut to 16:9 and 4:5 (`press-pack/manifest.yaml`, originals in the gitignored `press-pack/photos/`), the kit of the next concert (`src/content/press-kits/`: release, programme and biogram PDFs, announcements, post, poster graphics, and the designer's print PDF from `press-pack/posters/` when there is one), the logotype, the readme and invoicing sheet, two ZIPs, and `index.json`. **Build it where the originals are and upload `public/press/` to the build host before deploying.** A host without it renders `/press` with its texts and a "write to us" mailto; a host with an index cut from other sources fails the build on `kitHash` (`lib/pressPack.ts`), because the page's copy buttons would disagree with the files beside them. The script refuses to build and names what it is waiting for, rather than shipping a pack with a gap.
